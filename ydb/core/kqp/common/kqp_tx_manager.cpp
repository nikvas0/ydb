...
public:
    ...
    bool AddLock(ui64 shardId, TKqpTxLock lock) override {
        ...
        if (auto lockPtr = shardInfo.Locks.FindPtr(lock.GetKey()); lockPtr) {
            ...
            lockPtr->LocksAcquireFailure |= isError;
        } else {
    ...
    bool HasValidSnapshot() const override {
        return ValidSnapshot;
    }

    void SetHasValidSnapshot(bool hasSnapshot) override {
    ...
    TCheckLocksResult CheckLocks() const override {
        ...
        result.Ok = true;
        if (HasValidSnapshot() && IsReadOnly()) {
    ...
    TPrepareInfo GetPrepareTransactionInfo(ui64 shardId) override {
        ...
        TPrepareInfo result {
            ...
            .Arbiter = std::nullopt,
            .Locks = {},
            .Sender = ((shardInfo.Source & ESource::BufferWriter)
                ? ESource::BufferWriter
                : ESource::Executer),
        };
